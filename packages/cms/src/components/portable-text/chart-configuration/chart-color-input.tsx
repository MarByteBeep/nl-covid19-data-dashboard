...
>((aggr: Record<string, string>, key: string) => {
	...
	if (typeof colorValue === 'string') {
		...
		Object.keys(colorValue)
			.filter(
				(x) =>
					typeof colorValue[x as keyof typeof colorValue] === 'string'
			)
			.forEach((x) => {
				const compoundKey = `${key}___${x}`;
				aggr[compoundKey] = colorValue[
					x as keyof typeof colorValue
				] as string;
			});
	}
	return aggr;
					...
					>
						<Box padding={4}>
							<Grid columns={[4, 6]} gap={[1, 1, 2, 3]}>
								{Object.entries(flatDataColors).map(
									([id, color]) => (
										<Flex
											key={id}
											direction="column"
								...
								>
									...
									<Radio
										checked={colorProperty === undefined}
										readOnly
									/>
								</Flex>
							</Grid>
