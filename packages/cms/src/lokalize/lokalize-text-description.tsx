		...
		<div>
			{isOpen && (
				<>
					...
					<p>
						Lokalize teksten zijn over het algemeen "domme" teksten
						zonder opmaak. Echter zijn er ook "slimme" teksten die
						opgemaakt kunnen worden met Markdown.
					</p>
					<p>
						Als op de cdb-dev-omgeving de "keys"-modus wordt
						aangezet (om de lokalize-keys te kunnen lezen), wordt
						bij elk veld wat Markdown ondersteunt een emoji ( ✅ )
						weergegeven. Deze velden zijn dus met markdown op te
						maken.
					</p>
					...
					<p>
						Met onze Markdown-implementatie is het mogelijk om een
						gele melding te tonen, hiervoor hebben we de
						"blockquote" syntax gekaapt.
						<pre
							style={{
								padding: 10,
								background: '#EEE',
              ...
              `}
					...
					<p>
						Met onze Markdown-implementatie is het mogelijk om tekst
						voor een enkele veiligheidsregio of gemeente weer te
						geven. Hiervoor hebben we de "codeblock" syntax gekaapt.
						Deze syntax werkt met drie tildes (~~~) of drie
			...
			<button onClick={() => setIsOpen((x) => !x)}>
				{isOpen ? 'Verberg' : 'Toon'} informatie over lokalize en
				Markdown
			</button>
		</div>
	);
