...
export function VaccineCoverageChoroplethPerGm({
	...
	const variables = {
		regio: siteText.vaccinaties.choropleth_vaccination_coverage.shared[
			selectedMap
		],
	};
	...
	return (
		<ChoroplethTile
			...
			legend={{
				...
				title: siteText.vaccinaties.choropleth_vaccination_coverage
					.shared.legend_title,
			}}
			metadata={{
			...
			{selectedMap === 'gm' && (
				<DynamicChoropleth
					...
					dataOptions={{
						isPercentage: true,
						getLink: (gmcode) =>
							reverseRouter.gm.vaccinaties(gmcode),
						tooltipVariables: {
							age_group:
			...
			{selectedMap === 'vr' && (
				<DynamicChoropleth
					...
					dataOptions={{
						isPercentage: true,
						getLink: (vrcode) =>
							reverseRouter.vr.vaccinaties(vrcode),
						tooltipVariables: {
							age_group:
								siteText.vaccinaties.age_groups[
									selectedAgeGroup
	...
	const secondaryContent = Object.entries(secondaryValues).map(
		([property, formattedValue]) => {
			...
			return (
				...
				>
					<Markdown
						content={replaceVariablesInText(content, tooltipVars)}
					/>
