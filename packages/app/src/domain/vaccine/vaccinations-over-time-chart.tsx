	...
	const coverageChartConfiguration:
		...
		return isDefined(coverageData)
			? ({
					...
					seriesConfig: [
						{
							label: text.grafiek_gevaccineerd_door_de_tijd_heen
								.label_totaal,
							shortLabel:
								text.grafiek_gevaccineerd_door_de_tijd_heen
	...
	const deliveryAndAdministrationChartConfiguration = useMemo(() => {
		return {
			...
			seriesConfig: [
				...
				>((x) => ({
					metricProperty:
						x as keyof VaccineDeliveryAndAdministrationsValue,
					type: 'stacked-area',
...
export function VaccinationChartControls(props: VaccinationChartControlsProps) {
	...
	const items: RadioGroupItem<ActiveVaccinationChart>[] = [
		...
		{
			label: siteText.charts.vaccination_coverage_controls
	...
	return (
		<RadioGroup value={initialChart} onChange={onChange} items={items} />
	);
}
