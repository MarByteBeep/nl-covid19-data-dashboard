	...
	return useMemo(() => {
		const getHref = (key: ItemKeys<T>) => {
			...
			if (layout === 'vr' && isPresent(code)) {
				return reverseRouter.vr[route as keyof ReverseRouter['vr']](
					code
				);
			}

			if (layout === 'gm' && isPresent(code)) {
				return reverseRouter.gm[route as keyof ReverseRouter['gm']](
					code
				);
			}
		};
		...
		const expandMap = (map: SidebarMap<T>): ExpandedSidebarMap<T> =>
			map.map((x) =>
				typeof x === 'string' ? getItem(x) : getCategory(x)
			);
