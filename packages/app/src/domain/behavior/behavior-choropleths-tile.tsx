...
export function BehaviorChoroplethsTile({
	...
	const keysWithoutData = useMemo(() => {
		...
		const keysWithoutData = behaviorIdentifiers.filter(
			(item) =>
				!Object.keys(firstRegionData).find((a) => a.includes(item))
		);
		...
		const keysThatAreAllNull = behaviorIdentifiers.filter((key) => {
			return data.behavior.every((region) => {
				return (
					region[
						`${key}_compliance` as keyof VrCollectionBehavior
					] === null &&
					region[`${key}_support` as keyof VrCollectionBehavior] ===
						null
				);
			});
		});
		...
		<ChartTile title={title} description={description}>
			<Box spacing={4} height="100%">
				...
				<Box display="flex" flexWrap="wrap" spacing={{ _: 4, md: 0 }}>
					<ChoroplethBlock
						title={
							siteText.nl_gedrag.verdeling_in_nederland
								.compliance_title
						}
						data={data}
			...
			<Box position="relative">
				{keysWithoutData.includes(currentId) && (
					...
					>
						<Text
							textAlign="center"
							css={css({ maxWidth: '300px' })}
						>
							{
								siteText.nl_gedrag.verdeling_in_nederland
									.geen_beschikbare_data
							}
						</Text>
					</Box>
				)}
				<ErrorBoundary>
					<DynamicChoropleth
						...
						dataConfig={{
							metricName: 'behavior',
							metricProperty:
								metricProperty as unknown as KeysOfType<
									VrCollectionBehavior,
						...
						formatTooltip={(context) => {
							...
							if (keysWithoutData.includes(currentId))
								return null;
