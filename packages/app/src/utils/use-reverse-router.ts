...
export function useReverseRouter() {
	...
	return useMemo(() => {
		const reverseRouter = {
			...
			in: {
				index: () =>
					reverseRouter.in.positiefGetesteMensen() + openMenuSuffix,
				positiefGetesteMensen: () =>
					`/internationaal/positief-geteste-mensen`,
			...
			vr: {
				index: (code?: string) =>
					code
						...
						: '/veiligheidsregio',
				maatregelen: (code: string) =>
					`/veiligheidsregio/${code}/maatregelen`,
				vaccinaties: (code: string) =>
					`/veiligheidsregio/${code}/vaccinaties`,
