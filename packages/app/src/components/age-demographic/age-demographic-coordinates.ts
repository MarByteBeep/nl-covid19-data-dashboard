...
>(
	...
	const rightPercentage = (value: T) =>
		percentage(value[rightMetricProperty]);
	...
	const leftPercentagePoint = createPoint(
		leftPercentageScale,
		leftPercentage
	);
	...
	const getTooltipCoordinates: GetTooltipCoordinates<T> = (
		...
		if (!isSmallScreen && event) {
			...
			if (infectedPercentageSide) {
				...
				left =
					width / 2 + axisWidth / 2 + rightPercentagePoint(value) / 2;
			} else {
