...
(async function () {
	...
	fs.writeFileSync(
		path.join(process.cwd(), '../cms/src/data/data-structure.ts'),
		code,
		{
			encoding: 'utf-8',
		}
...
function constructDataStructure(schema: any, schemaName: string) {
	...
	if (schemaName.endsWith('_collection')) {
		Object.keys(properties).forEach((x) => {
			if (
				!isDefined(properties[x]?.properties) ||
				skippedProperties.includes(x)
			) {
				delete properties[x];
			} else {
				const valuesProperties = properties[x]?.properties;
				properties[x] =
					Object.keys(valuesProperties).filter(skipProperties);
			}
		});
	} else {
		Object.keys(properties).forEach((x) => {
			if (
				!isDefined(properties[x]?.properties?.values) ||
				skippedProperties.includes(x)
			) {
				delete properties[x];
			} else {
				...
				const valuesProperties =
					properties[x]?.properties?.values?.properties;
				properties[x] =
					Object.keys(valuesProperties).filter(skipProperties);
			}
		});
...
function generateCodeStructure(schemaInfo: Record<string, any>) {
	...
	const propertyAssignments = Object.entries(schemaInfo).map(([key, value]) =>
		createPropertyValue(key, value)
	);
	...
	const propertyAssignments = Object.keys(value).map((x) => {
		return ts.factory.createPropertyAssignment(
			...
			ts.factory.createArrayLiteralExpression(
				value[x].map((y: string) => ts.factory.createStringLiteral(y))
			)
		);
	});
